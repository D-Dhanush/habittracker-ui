...
@Component({
  ...
  `,
  styles: [
    `.toast-container { position: fixed; right: 1.5rem; bottom: 1.5rem; display: grid; gap: 0.85rem; z-index: 5000; }`,
    `.toast { min-width: 240px; max-width: 360px; padding: 1rem 1.15rem; border-radius: 18px; color: white; font-weight: 600; box-shadow: 0 18px 40px rgba(0, 0, 0, 0.35); cursor: pointer; border: 1px solid rgba(255, 255, 255, 0.12); background: rgba(10, 14, 24, 0.92); animation: toastGlow 2.5s ease infinite; }`,
    ...
    `@keyframes toastGlow { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-2px); } }`
  ]
})
